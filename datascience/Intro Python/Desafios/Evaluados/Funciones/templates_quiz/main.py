...
while correcto and n_pregunta < 3*p_level:
    if n_pregunta == 0:
        ...
        while p_level not in range(1,4):
            p_level = int(input('Ingrese un número válido (Máximo 3): '))
    ...
    if continuar == 'y':
        ...
        while respuesta not in ['a', 'b', 'c','d']:
            respuesta = input('Escoja una alternativa válida: ').lower()
        ...
        if correcto and n_pregunta < 3*p_level:
            ...
            while continuar not in ('y','n'):
                continuar = input('Ingrese una opción válida [y/n]: ').lower()
            os.system(op_sys)
